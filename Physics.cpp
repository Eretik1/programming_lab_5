...
void Physics::update(std::vector<Ball>& balls, const size_t ticks, std::vector<Ball>& dusts) const {
...
void Physics::collideBalls(std::vector<Ball>& balls, std::vector<Ball>& dusts) const {
    for (auto a = balls.begin(); a != balls.end(); ++a) {
        for (auto b = std::next(a); b != balls.end(); ++b) {
            ...
            if (distanceBetweenCenters2 < collisionDistance2 && ((a->getCollidable() == true) && (b->getCollidable() == true))) {
                processCollision(*a, *b, distanceBetweenCenters2);
                
                Dust dust((b->getCenter() + a->getCenter()) / 2);
                std::vector<Ball> dustss = dust.getBalls();
                for(int i = 0; i < dustss.size(); i++){
                    dusts.push_back(dustss[i]);
                }
...
void Physics::collideWithBox(std::vector<Ball>& balls, std::vector<Ball>& dusts) const {
    for (Ball& ball : balls) {
...
void Physics::move(std::vector<Ball>& balls, std::vector<Ball>& dusts) const {
    for (Ball& ball : balls) {
        Point newPos =
